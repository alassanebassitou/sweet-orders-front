...
import { mockCommandes, statutOrder } from '@/lib/mockData';
...
export default function ClientCommandeDetail() {
  ...
  const cmd = mockCommandes.find((c) => c.id === id);

  if (!cmd) {
    return (
      <div className="p-6 text-center">
        <p className="text-muted-foreground">Commande introuvable</p>
        <Button onClick={() => navigate('/app/commandes')} className="mt-4">Retour</Button>
      </div>
    );
  }

  const currentStep = statutOrder.indexOf(cmd.statut);
  const reste = cmd.montantTotal - cmd.paye;
  ...
  return (
    <div className="p-4 md:p-6 space-y-5 animate-fade-in">
      ...
      <div>
        ...
        <p className="text-muted-foreground text-sm">Passée le {formatDate(cmd.dateCommande)}</p>
      </div>
      ...
      {cmd.statut === 'PRETE' && (
        <Card className="bg-purple-50 border-purple-200">
          <CardContent className="p-4 text-sm text-purple-800">🎉 Votre commande est prête ! La livraison arrive bientôt.</CardContent>
        </Card>
      )}
      {cmd.statut === 'LIVREE' && (
        <Card className="bg-success/10 border-success/30">
          <CardContent className="p-4 text-sm text-success">✅ Commande livrée. Merci pour votre confiance !</CardContent>
        </Card>
      )}
        ...
        <CardContent className="p-4 space-y-3">
          ...
          {cmd.produits.map((p, i) => (
            <div key={i} className="text-sm">
              ...
              {p.personnalisations?.length > 0 && (
                <p className="text-xs text-muted-foreground">Options : {p.personnalisations.join(', ')}</p>
              )}
            </div>
          ))}
        </CardContent>
      ...
      <Card>
        <CardContent className="p-4 space-y-1 text-sm">
          ...
          <div className="flex justify-between"><span>Montant total</span><span className="font-semibold">{formatFCFA(cmd.montantTotal)}</span></div>
          <div className="flex justify-between text-success"><span>✓ Acompte payé</span><span>{formatFCFA(cmd.paye)}</span></div>
          {reste > 0 && (
            <>
      ...
      <Button variant="outline" onClick={() => { toast.success('Commande dupliquée'); navigate('/app/commander'); }} className="w-full gap-2">
