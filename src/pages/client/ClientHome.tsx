    ...
    <div className="p-4 md:p-6 space-y-8 animate-fade-in">
      <div>
        <h1 className="font-display text-2xl md:text-3xl font-bold">
          Bonjour {user?.prenom || user?.nom || user?.name} 👋
        </h1>
        <p className="text-muted-foreground">Que souhaitez-vous commander aujourd'hui&nbsp;?</p>
      </div>

      <div className="grid sm:grid-cols-2 gap-4">
        <button onClick={() => navigate('/app/catalogue')} className="group p-6 rounded-2xl bg-gradient-to-br from-primary/15 to-primary/5 border border-border text-left hover:shadow-md transition-all">
        ...
        <button onClick={() => navigate('/app/commandes')} className="group p-6 rounded-2xl bg-gradient-to-br from-accent/10 to-accent/5 border border-border text-left hover:shadow-md transition-all">
      ...
      <section>
        ...
        {cmdsQ.isLoading ? (
          ...
          <Card><CardContent className="p-8 text-center text-muted-foreground"><p>Aucune commande en cours</p></CardContent></Card>
        ) : (
